...
import {
  ArrowRight, Zap, Layers, Globe, Terminal, BarChart3, Code2,
  GitBranch, Shield, Check, X, Minus, MessageSquare, Bug,
  BookOpen, Star, Heart, Cpu, Gauge, FileCode, Braces,
  LineChart, Settings2, ChevronRight, Play
} from "lucide-react";

import lockupHLight from "@/assets/logo-lockup-horizontal-light.png";
import heroBench from "@/assets/polybench-hero-bench-1.png";
...
const problems = [
  { icon: X, text: "No unified cross-language benchmarking — separate harnesses, different APIs, different methodologies" },
  { icon: X, text: "Fair comparisons are hard — different iteration counts, warmup strategies, and data inputs" },
  { icon: X, text: "Results are scattered — each language's output looks different, side-by-side comparisons are tedious" },
];

const solutions = [
  { icon: Check, text: "Define once, run everywhere — a custom DSL (.bench files) compiles to native code in each language" },
  { icon: Check, text: "Unified measurement — same iterations, same data, same warmup for apples-to-apples comparisons" },
  { icon: Check, text: "Rich output — console tables, markdown, JSON for CI, and SVG charts from a single run" },
];

const pipeline = [
  { step: "01", title: "Parse", desc: "Your .bench file → Lexer → AST → Semantic validation", icon: FileCode, color: "from-red-500 to-red-400" },
  { step: "02", title: "Lower to IR", desc: "Resolve fixtures, merge config defaults, normalize imports", icon: Braces, color: "from-orange-500 to-amber-400" },
  { step: "03", title: "Generate", desc: "Emit native Go, TypeScript, and Rust benchmark code", icon: Code2, color: "from-violet-500 to-indigo-400" },
  { step: "04", title: "Execute", desc: "Isolated subprocesses — no FFI, full native speed per language", icon: Play, color: "from-blue-500 to-cyan-400" },
  { step: "05", title: "Report", desc: "Console tables, markdown docs, JSON, SVG charts", icon: LineChart, color: "from-emerald-500 to-green-400" },
];

const features = [
  { icon: FileCode, title: "Custom DSL", desc: "Clean, declarative syntax with language-specific setup blocks" },
  { icon: Gauge, title: "Auto-calibration", desc: "Automatically adjusts iteration counts to reach target execution time" },
  { icon: Cpu, title: "Memory Profiling", desc: "Track allocations across Go, TypeScript, and Rust" },
  { icon: Layers, title: "Portable Fixtures", desc: "Share hex-encoded test data across all language implementations" },
  { icon: BarChart3, title: "Chart Generation", desc: "SVG bar charts, pie charts, and line charts from results" },
  { icon: Settings2, title: "LSP Support", desc: "Full editor integration with diagnostics, completions, and formatting" },
];

const crates = [
  { name: "poly-bench-dsl", desc: "Lexer, parser, AST, formatter, validator" },
  { name: "poly-bench-ir", desc: "Intermediate representation — normalized benchmark structures" },
  { name: "poly-bench-runtime", desc: "Language-specific code generation and execution" },
  { name: "poly-bench-executor", desc: "Orchestrates runs, calibration, measurements" },
  { name: "poly-bench-reporter", desc: "Console, markdown, JSON, SVG output" },
  { name: "poly-bench-lsp", desc: "Language Server Protocol for editor support" },
  { name: "poly-bench-stdlib", desc: "Standard library — anvil, charting, constants" },
  { name: "poly-bench-project", desc: "Project init, dependency management, manifests" },
];

const runtimeDetails = [
  { lang: "Go", runtime: "Plugin-based execution or subprocess", memory: "runtime.ReadMemStats", accent: "#00ADD8" },
  { lang: "TypeScript", runtime: "Node.js subprocess", memory: "process.memoryUsage()", accent: "#3178C6" },
  { lang: "Rust", runtime: "Cargo build + subprocess", memory: "Custom allocator tracking", accent: "#DEA584" },
];

const installScriptUrl = "https://raw.githubusercontent.com/evm-tooling/poly-bench/main/scripts/install.sh";
  ...
  return (
    <>
      ...
      <main className="relative overflow-x-clip">
            ...
            <div className="grid lg:grid-cols-[1fr_1.1fr] gap-16 items-start">
              ...
              >
                ...
                >
                  ...
                  <strong className="text-foreground">Rust</strong>. Get unified, statistically sound results.
                </motion.p>
              ...
              >
                ...
                <div className="rounded-2xl overflow-hidden border border-border bg-code-bg shadow-xl dark:shadow-none">
                  <div className="flex items-center gap-2 px-4 py-3 bg-background-elevated border-b border-border">
                    ...
                    <span className="text-xs ml-2 font-mono text-foreground-muted">keccak.bench</span>
                  </div>
                  <Highlight theme={codeTheme} code={benchFileCode.trim()} language="bench">
                ...
                <div className="rounded-2xl overflow-hidden border border-border bg-code-bg shadow-xl dark:shadow-none">
                  <div className="flex items-center justify-between gap-2 px-4 py-1 bg-background-elevated border-b border-border">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="w-3 h-3 rounded-full bg-amber-400" />
                      <span className="w-3 h-3 rounded-full bg-emerald-400" />
                      <span className="text-xs ml-2 font-mono text-foreground-muted">Terminal</span>
                    </div>
                    <CopyButton text={installCommand} />
                  </div>
                  <Highlight theme={codeTheme} code={`$ ${installCommand}`.trim()} language="bash">
                    {({ tokens, getLineProps, getTokenProps }) => (
        ...
        {/* ── THE PROBLEM / SOLUTION — toggle tabs (accent section) ── */}
        <section id="how-it-works" className="py-28 bg-background-tertiary text-foreground border-t border-border">
          <div className="max-w-5xl mx-auto px-6">
            ...
            >
              ...
              <p className="mt-4 max-w-2xl mx-auto text-foreground-secondary">
              ...
              <button
                ...
                className={`text-sm font-medium px-5 py-2.5 rounded-lg transition-all ${
                  activeTab === "problem"
                    ...
                    : "text-foreground-muted border border-transparent hover:text-foreground-secondary"
                }`}
              >
                The Problem
              </button>
            ...
            <div className="grid md:grid-cols-3 gap-5">
              {(activeTab === "problem" ? problems : solutions).map((item, i) => (
                ...
                >
                  <item.icon
                    className={`w-5 h-5 mb-3 ${activeTab === "problem" ? "text-destructive" : "text-success"}`}
                  />
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <h3 className="text-3xl md:text-4xl font-black tracking-tight text-foreground">
                  One run.<br />Every language.<br />
                  <span className="text-foreground-muted">Compared.</span>
                </h3>
                <p className="mt-4 leading-relaxed text-foreground-secondary">
                  Run{" "}
                  <code className="px-1.5 py-0.5 rounded text-sm bg-tertiary/10 text-tertiary">
                    poly-bench run
                  </code>{" "}
                  and get a statistical comparison across all three languages. Includes mean, standard
                  deviation, ops/sec, and speedup ratios.
                </p>
                <div className="mt-6 flex flex-wrap gap-3">
                  {["Console", "Markdown", "JSON", "SVG Charts"].map((fmt) => (
                    <span
                      key={fmt}
                      className="text-xs rounded-full px-3 py-1 border border-border text-foreground-muted"
                    >
                      {fmt}
                    </span>
                  ))}
              ...
              <div className="rounded-2xl overflow-hidden border border-border bg-code-bg-deep">
                ...
                <pre className="p-5 text-[13px] leading-relaxed font-mono overflow-x-auto text-terminal-output">
              ...
              {pipeline.map((p, i) => (
                ...
                >
                  <div className="rounded-2xl p-6 h-full transition-all hover:-translate-y-1 border border-border bg-card hover:shadow-lg">
                    <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${p.color} text-white flex items-center justify-center mb-4`}>
                      <p.icon className="w-5 h-5" />
                    </div>
                    <span className="text-[10px] font-bold uppercase tracking-widest text-foreground-muted">
                      {p.step}
                    </span>
                    <h4 className="font-bold mt-1 text-foreground">{p.title}</h4>
                    <p className="text-xs mt-2 leading-relaxed text-foreground-secondary">{p.desc}</p>
                  </div>
                  {i < pipeline.length - 1 && (
          ...
          <div className="max-w-5xl mx-auto px-6">
            ...
            <div className="grid md:grid-cols-3 gap-5">
              {runtimeDetails.map((r, i) => (
                ...
                >
                  <div className="absolute top-0 left-0 w-full h-1" style={{ backgroundColor: r.accent }} />
                  <h4 className="font-bold text-lg" style={{ color: r.accent }}>{r.lang}</h4>
                  <dl className="mt-4 space-y-3 text-sm">
                    <div>
                      <dt className="text-xs uppercase tracking-widest text-foreground-muted">Runtime</dt>
                      <dd className="mt-0.5 text-foreground-secondary">{r.runtime}</dd>
                    </div>
                    <div>
                      <dt className="text-xs uppercase tracking-widest text-foreground-muted">Memory</dt>
                      <dd className="mt-0.5 font-mono text-xs text-foreground-secondary">{r.memory}</dd>
                    </div>
                  </dl>
                </motion.div>
              ))}
          ...
          <div className="max-w-6xl mx-auto px-6">
            <div className="grid lg:grid-cols-[1fr_2fr] gap-16 items-start">
              <div className="lg:sticky lg:top-28">
                ...
                <p className="mt-4 leading-relaxed text-foreground-secondary">
                  poly-bench provides a complete toolkit — from DSL parsing to chart generation — built in
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {features.map((f, i) => (
                  ...
                  >
                    ...
                    <p className="text-sm mt-1 leading-relaxed text-foreground-secondary">{f.desc}</p>
                  </motion.div>
                ))}
              </div>
            </div>
        ...
        {/* ── STATISTICS ── */}
        <section className="py-28 bg-background-tertiary border-t border-border">
          <div className="max-w-5xl mx-auto px-6">
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              <div>
                ...
                <h3 className="text-3xl md:text-4xl font-black tracking-tight text-foreground">
                  No shortcuts on<br />measurement.
                </h3>
                <p className="mt-4 leading-relaxed text-foreground-secondary">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  { title: "Warmup Removal", desc: "Discard initial iterations before timing" },
                  { title: "Multi-run", desc: "Run benchmarks N times for reliable stats" },
                  { title: "Outlier Detection", desc: "IQR-based outlier removal" },
                  { title: "Auto-calibration", desc: "Hit target execution time automatically" },
                ].map((s, i) => (
                  <motion.div
                    key={s.title}
                    initial="hidden"
                    ...
                    custom={i}
                    variants={fadeUp}
                    className="rounded-xl p-5 border border-border bg-background-elevated"
                  >
                    <h4 className="font-semibold text-sm text-foreground">{s.title}</h4>
                    <p className="text-xs mt-1 leading-relaxed text-foreground-muted">{s.desc}</p>
                  </motion.div>
                ))}
              </div>
            </div>
          ...
          <div className="max-w-5xl mx-auto px-6">
            <div className="grid lg:grid-cols-[1fr_1fr] gap-12">
              ...
              <div className="flex flex-col gap-3">
                {[
                  { icon: MessageSquare, title: "Discussions", desc: "Ask questions and share ideas", href: "https://github.com/evm-tooling/poly-bench/discussions" },
                  { icon: Bug, title: "Issues", desc: "Report bugs or request features", href: "https://github.com/evm-tooling/poly-bench/issues" },
                  { icon: BookOpen, title: "Docs", desc: "Full documentation and guides", href: "/docs/getting-started" },
                ].map((item) => (
                  <a
                    key={item.title}
                    href={item.href}
