...
For the high-level flow, see [Architecture](/docs/architecture). For user-facing setup and gotchas, see [Requirements](/docs/requirements). To add a new runtime, see [Adding a Runtime](/docs/developers/adding-a-runtime).
...
|----------|------------------|
| [Go](/docs/architecture/polybench-runtime/go) | `go.mod`, codegen, plugin vs subprocess, `polybench_runner` |
| [TypeScript](/docs/architecture/polybench-runtime/ts) | `package.json`, `tsconfig.json`, V8/Node execution, `bench.mjs` |
| [Rust](/docs/architecture/polybench-runtime/rust) | `Cargo.toml`, `main.rs` overwrite, `polybench_runner` binary |
| [Python](/docs/architecture/polybench-runtime/python) | `.venv`, `requirements.txt`, `bench.py`, subprocess execution |
| [C](/docs/architecture/polybench-runtime/c) | `clang` vs CMake+vcpkg, generated `.c`, build flow |
| [C#](/docs/architecture/polybench-runtime/csharp) | `polybench.csproj`, `Program.cs` overwrite, `global.json`, `dotnet run` |
| [Zig](/docs/architecture/polybench-runtime/zig) | `build.zig`, `build.zig.zon`, generated `.zig`, `zig build` |
