...
```bash
poly-bench run benchmarks/hash.bench \
  --lang go \
  --report json \
  --output out/
```
...
```bash
poly-bench check <FILE> [--show-ast]
```
...
```bash
poly-bench compile [FILE] [--lang <LANG>] [--no-cache] [--clear-cache]
```
...
```bash
poly-bench codegen <FILE> --lang <LANG> --output <DIR>
```
...
```bash
poly-bench init [NAME] [--languages go,ts,rust] [--no-example]
```
...
```bash
poly-bench build [--force] [--skip-install]
```
