...
| Embedded languages | Delegates to gopls, tsserver, rust-analyzer, pyright/pylsp, clangd, csharp-ls, and zls for code blocks |
  ...
  tabs={[
    {
      title: "Terminal",
      language: "bash",
      code: `poly-bench lsp  # [!code focus]`
    },
  ]}
/>
  ...
  tabs={[
    {
      ...
      language: "json",
      code: `{
  "poly-bench.serverPath": "poly-bench",  // [!code focus:2]
...
if not configs.polybench then
  configs.polybench = {
    default_config = {
      cmd = { 'poly-bench', 'lsp' },  -- [!code focus:2]
      filetypes = { 'bench' },
...
syn keyword benchOption concurrency outlierDetection cvThreshold
