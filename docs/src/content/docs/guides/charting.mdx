...
description: How to generate bar charts, line charts, speedup charts, and tables from benchmark results
---
...
| Chart | Best for |
|-------|----------|
| `drawBarChart` | Comparing all languages side-by-side across multiple benchmarks |
| `drawLineChart` | Showing how performance scales as input size grows |
...
The simplest possible bar chart — just a title and output file.
...
Line charts are designed for scaling benchmarks where benchmark names encode a numeric size. poly-bench extracts the number from the name (e.g. `n100` → 100, `size1024` → 1024) and uses it as the x-axis value.
...
Shows the speedup ratio of each language relative to a baseline. A value of 2.0× means that language is twice as fast as the baseline for that benchmark.
