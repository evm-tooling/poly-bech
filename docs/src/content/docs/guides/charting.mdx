...
| Chart | Best for |
|-------|----------|
| `drawTable` | Comparing all languages side-by-side; embedding results in READMEs or reports |
| `drawSpeedupChart` | Showing how performance scales as input size grows; highlighting speedup vs baseline |

All four require `use std::charting` at the top of the file and must be called inside an `after { }` block.

<Aside type="note">
<span>All chart directives require <code>use std::charting</code> at the top of the file and must be called inside an <code>after &#123; &#125;</code> block.</span>
</Aside>
...
Show memory allocation data alongside timing. Use `suiteType: memory` in the suite declaration.
  ...
  tabs={[
    {
      title: "bench",
      language: "bench",
      code: `declare suite cryptoBench performance timeBased sameDataset: true {
    count: 10
    ...
    after {
        charting.drawTable(
            ...
            showErrorBars: true,  # [!code focus:4]
            ciLevel: 95,
            errorBarOpacity: 0.8,
...
<CodeGroup
  tabs={[
    ...
    charting.drawTable(
        ...
        xlabel: "Array Size",
        showRegression: true,  # [!code focus:5]
...
declare suite sortN performance timeBased sameDataset: true {
    targetTime: 500ms
  ...
  tabs={[
    {
      title: "bench",
      language: "bench",
      code: `declare suite matmulN performance timeBased sameDataset: true {
    count: 5
...
<CodeGroup
  tabs={[
    ...
    charting.drawSpeedupChart(
        ...
        xlabel: "Payload Size",
        chartMode: "throughput",  # [!code focus:2]
        timeUnit: "s"
    )
}`
    },
...
<CodeGroup
  tabs={[
    ...
    charting.drawSpeedupChart(
        ...
        baselineBenchmark: "hashShort",  # [!code focus:1]
        sortBy: "speedup",
        sortOrder: "desc",
        showGrid: true,
        gridOpacity: 0.15,
...
<Aside type="tip">
<span>You can call any number of chart directives in a single <code>after</code> block; each produces a separate SVG file.</span>
</Aside>
...
declare suite sortN memory timeBased sameDataset: true {
...
<Aside type="caution">
<span>Charts are only written when you pass <code>--output</code> to the CLI. Without it, charts are not generated even if <code>after &#123; &#125;</code> directives are present.</span>
</Aside>
