  ...
  tabs={[
    {
      title: "Terminal",
      language: "bash",
      code: `poly-bench run benchmarks/keccak.bench  # [!code focus]`
    },
  ]}
/>
  ...
  tabs={[
    {
      title: "Terminal",
      language: "bash",
      code: `poly-bench run  # [!code focus]`
    },
  ]}
/>
...
# Run only Go
poly-bench run benchmarks/keccak.bench --lang go  # [!code focus:2]
...
poly-bench run benchmarks/keccak.bench --iterations 100  # [!code focus:1]
...
poly-bench fmt benchmarks/keccak.bench --write  # [!code focus:2]
...
<Aside type="note">
<span><code>codegen</code> supports only Go, TypeScript, and Rust. Python, C, C#, and Zig do not emit standalone harness files.</span>
</Aside>
...
| CI validation | `for f in benchmarks/*.bench; do poly-bench check "$f"; done` |
...
- [Reporting](/docs/getting-started#reporting) — Output formats, chart generation, and CI integration (in Getting Started)
