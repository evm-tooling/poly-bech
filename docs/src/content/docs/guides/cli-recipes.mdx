...
poly-bench run benchmarks/keccak.bench --report markdown --report json --output results/`
    },
  ]}
/>
  ...
  tabs={[
    {
      title: "Terminal",
      language: "bash",
      code: `# Format a single file (in-place)
poly-bench fmt benchmarks/keccak.bench

# Format all .bench files in benchmarks/
poly-bench fmt
...
# Inspect the output
cat generated/benchmark_gen.go`
    },
  ]}
/>
...
Use `--verbose` to see per-iteration timing, calibration decisions, and other diagnostic information.
  ...
  tabs={[
    {
      title: "Terminal",
      language: "bash",
      code: `poly-bench run benchmarks/keccak.bench --verbose`
    },
  ]}
/>
  ...
  tabs={[
    {
      title: "Output",
      language: "text",
      code: `[keccak] Calibrating go...
  trial 1: 1000 iters → 2.3ms (target: 3000ms)
  trial 2: 100000 iters → 231ms (target: 3000ms)
  trial 3: 1300000 iters → 3001ms ✓
[keccak] Running go: 1300000 iters × 3 runs
  run 1: 2.31 ns/op (CV: 1.2%)
  run 2: 2.29 ns/op (CV: 0.8%)
  run 3: 2.33 ns/op (CV: 1.5%)
  result: 2.31 ns/op (p50), 2.29 ns/op (p95)`
    },
  ]}
/>
...
jobs:
  bench:
    ...
    steps:
      ...
      - name: Run benchmarks
        run: |
          poly-bench run --report markdown --report json --output results/
...
| Full run with all outputs | `poly-bench run bench.bench --report markdown --report json --output results/` |
| CI format check | `poly-bench fmt --check` |
| CI validation | `poly-bench check benchmarks/*.bench` |
| Debug slow benchmark | `poly-bench run bench.bench --verbose --lang go` |
| Inspect generated Go code | `poly-bench codegen bench.bench --lang go --output gen/` |
| Run without charts | `poly-bench run bench.bench` (omit `--output`) |
