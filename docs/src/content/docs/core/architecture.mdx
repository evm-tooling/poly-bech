...
| `poly-bench-runtime` | Language-specific code generation and runtime execution (Go, TS, Rust, Python, C, C#, Zig) |
...
See [Polybench Runtime](/docs/polybench-runtime) for per-language technical deep-dives on scaffolding, codegen, and execution.
...
| Python | Python subprocess | `tracemalloc` |
| C, C#, Zig | Native build + subprocess | Per-runtime |
...
<FlowDiagram
  ...
  branches={[
    ...
    },
    {
      ...
      label: "node benchmark.ts → collect timing + memory"
    },
    {
      condition: "Rust",
    ...
    {
      condition: "Python",
      label: "python script.py → collect timing + memory"
    }
  ]}
/>
...
When `runMode` is `timeBased` (in the suite declaration), poly-bench calibrates the iteration count to hit a target execution time:
...
Standard library modules are implemented in Rust and invoked via directives in `globalSetup` and `after` blocks:
...
declare suite example performance timeBased sameDataset: true {
    ...
    after {
        charting.drawTable(title: "Results")
        anvil.stopAnvil()
    }
}`
    },
