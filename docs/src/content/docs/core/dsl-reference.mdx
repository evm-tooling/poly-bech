...
<Aside type="note">
<span><code>benchAsync</code> applies internal caps for practical runtime behavior: warmup ≤ 5 and samples ≤ 50. Async implementations should return a value so errors/no-ops are not silently hidden.</span>
</Aside>
