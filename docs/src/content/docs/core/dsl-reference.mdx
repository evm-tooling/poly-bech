...
# One or more suites (declare suite is required)
  ...
  tabs={[
    {
      title: "Imports",
      language: "bench",
      code: `use std::charting   # Chart generation  # [!code focus:3]
...
globalSetup {
    ...
    },
    {
      title: "Suite-level",
      language: "bench",
      code: `declare suite evmBench performance timeBased sameDataset: true {
...
<CodeGroup 
  tabs={[
    ...
    baseline: "go"            # Language for speedup ratios ("go", "ts", "rust", "python", "c", "csharp", "zig")
...
<Aside type="caution">
<span>The property <code>mode</code> is deprecated and rejected. Use <code>declare suite</code> with <code>runMode</code> (<code>timeBased</code> or <code>iterationBased</code>) in the header instead.</span>
</Aside>
  ...
  tabs={[
    {
      title: "Examples",
      language: "bench",
      code: `declare suite timing performance timeBased sameDataset: true {
...
Setup blocks define language-specific imports, package-level declarations, one-time initialization, and helper functions. Supported language keywords: `go`, `ts` (or `typescript`), `rust`, `python`, `c`, `csharp`, `zig`.
...
<CodeGroup 
  tabs={[
    ...
    go: sha256Sum(data)  # [!code focus:3]
    ts: sha256Sum(data)
    ...
    },
    {
      ...
      language: "bench",
      code: `benchAsync fetchBlock {  # [!code focus:3]
    go: getBlockNumber()
...
- [Features](/docs/features) — Fixtures, hooks, globalSetup, charting reference
- [CLI Reference](/docs/tools/cli) — Command-line options
- [Architecture](/docs/architecture) — How the DSL compiles to native code
