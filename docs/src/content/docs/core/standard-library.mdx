...
suite example {
    ...
    after {
        ...
        charting.drawSpeedupChart(title: "Scaling", output: "results-line.svg")
...
Generates a grouped table comparing benchmark results across languages. This is the most feature-rich chart type with the full set of available parameters.
...
Generates a speedup chart showing how performance scales across benchmarks. Works best when benchmark names encode a numeric size (e.g. `n100`, `n200`, `size8`, `size16`) — poly-bench extracts the number for the x-axis.

`drawSpeedupChart` supports the same parameter groups as `drawTable` with the following differences:
...
##### `drawSpeedupChart`-only Parameter

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|

All other parameters from `drawTable` are available on `drawSpeedupChart` except the bar-specific and display-toggle ones listed above.
...
| `std::charting` | `use std::charting` | `drawTable()`, `drawSpeedupChart()`, `drawSpeedupChart()`, `drawTable()` |
