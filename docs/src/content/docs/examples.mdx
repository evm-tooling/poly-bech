...
<Aside type="note">
<span>Use <code>benchAsync</code> for async operations. It runs in async-sequential mode (one awaited call per iteration), applies internal caps (<code>warmup &lt;= 5</code>, <code>samples &lt;= 50</code>), and expects implementations to return a value.</span>
</Aside>
