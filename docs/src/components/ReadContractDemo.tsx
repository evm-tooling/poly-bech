...
export default function ReadContractDemo({
  ...
  const [lines, setLines] = React.useState<TerminalLine[]>([
    {
      kind: 'meta',
      text: 'Click Run to execute a real readContract against RPC.',
    },
  ])
  ...
  const run = React.useCallback(async () => {
    ...
    setLines([
      ...
      {
        kind: 'command',
        text: buildCommand(rpcUrl, tokenAddress, userAddress),
      },
    ...
    try {
      ...
      if (!res.ok) {
        const msg =
          'error' in json ? json.error : `Runner failed (${res.status})`
        throw new Error(msg)
      }
      ...
      setLines([
        ...
        {
          kind: 'stdout',
          text: `Balance formatted: ${json.formattedBalance} (decimals=${json.decimals})`,
        },
      ...
      <div className="flex items-center justify-between h-10 bg-code-bg/50 border-b border-code-border px-2">
        <div className="flex items-stretch min-w-0">
          ...
          <div className="flex items-center px-3 text-[0.8125rem] font-medium text-foreground bg-code-bg-deep/20 border-r border-code-border">
            <span className="text-[0.75rem] text-foreground-muted mr-2">
              Go
            </span>
