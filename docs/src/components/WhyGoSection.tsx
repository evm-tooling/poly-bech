...
const WhyGoSection = () => (
  <section className="relative overflow-hidden section-bg-dark py-24">
    ...
    <div
      aria-hidden
      className="glow-orb-ellipse top-1/2 left-1/2 -translate-x-1/2 -translate-y-[50%] h-[400px] w-[1200px] opacity-40 blur-[130px]"
    />
    <div
      aria-hidden
      className="glow-orb-ellipse top-1/2 left-1/2 -translate-x-1/2 -translate-y-[30%] h-[280px] w-[400px] opacity-25 blur-[80px]"
    />
        ...
        {reasons.map((r, i) => (
          <AnimatedSection key={r.title} delay={i * 0.1}>
            <Card
              ...
              padding="md"
              className="group relative h-full hover:-translate-y-1 transition-transform duration-200"
            >
              <div
                className={`absolute top-0 left-4 right-4 h-0.5 rounded-b-full ${r.accent === 'tertiary' ? 'bg-tertiary' : 'bg-primary'} opacity-60`}
              />
              <div
                className={`mb-4 icon-box h-12 w-12 rounded-xl ${
                  r.accent === 'tertiary'
                    ? 'icon-box-tertiary'
                    : 'icon-box-primary'
                }`}
