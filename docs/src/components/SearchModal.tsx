...
export default function SearchModal({ onClose }: { onClose: () => void }) {
    ...
    return index
      .filter(
        (entry) =>
          ...
          fuzzyMatch(entry.section, query) ||
          fuzzyMatch(entry.slug, query) ||
  ...
  return (
    <div
      className="fixed inset-0 z-[100] h-[100vh] bg-black/55 backdrop-blur-md flex items-start justify-center pt-[min(15vh,120px)] px-4 overflow-y-auto"
      onClick={onClose}
    >
      <div
        className="w-full max-w-[640px] bg-background-tertiary/95 border border-border rounded-xl shadow-2xl shadow-black/45 overflow-hidden flex flex-col max-h-[min(70vh,600px)] my-auto mt-0 mb-auto"
        role="dialog"
      ...
      >
        {/* Search input */}
        <div className="flex items-center gap-3 px-4 border-b border-border bg-background-secondary/80">
          <svg
          ...
          <input
            ...
            spellCheck={false}
          />
          <kbd className="hidden sm:flex items-center gap-0.5 text-[11px] text-foreground-muted bg-background/70 border border-border rounded px-1.5 py-0.5 font-mono">
            Esc
          </kbd>
        </div>
        ...
        <div
          ref={listRef}
          className="flex-1 overflow-y-auto overscroll-contain bg-background-tertiary/65"
        >
          {query.trim() === '' ? (
        ...
        {/* Footer */}
        <div className="flex items-center gap-4 px-4 py-2.5 border-t border-border bg-background-secondary/65 text-[11px] text-foreground-muted">
          <span className="flex items-center gap-1">
            <kbd className="bg-background/70 border border-border rounded px-1 py-px font-mono">
              &uarr;
            </kbd>
            <kbd className="bg-background/70 border border-border rounded px-1 py-px font-mono">
              &darr;
