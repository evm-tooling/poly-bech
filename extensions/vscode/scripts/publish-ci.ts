...
async function main() {
  ...
  if (!version) {
    console.error("Usage:");
    console.error("  VERSION=0.0.22 npm run publish:ci");
    console.error("  npm run publish:ci -- VERSION=0.0.22");
    process.exit(1);
  }
  ...
  if (!vscePat) {
    console.error("❌ VSCE_PAT environment variable is required for publishing");
    process.exit(1);
  }
  ...
  // 5) vsce publish (this packages AND publishes in one step)
  console.log("📤 vsce publish…");
  await run("npx", ["--yes", "@vscode/vsce", "publish", "--pat", vscePat], root);

  console.log("✅ Published to VS Code Marketplace!");
}

main().catch((err) => {
  console.error("❌ publish:ci failed:", err instanceof Error ? err.stack ?? err.message : err);
  process.exit(1);
});
