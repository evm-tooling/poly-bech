...
use runtimes_csharp::CSharpRuntime;
...
fn minimal_suite_fixed() -> (BenchmarkSpec, SuiteIR) {
    ...
    spec.implementations.insert(Lang::CSharp, "data.Length".to_string());
...
#[tokio::test]
#[ignore = "requires dotnet SDK in PATH"]
async fn test_csharp_fixed_mode_execution() {
    let (spec, suite) = minimal_suite_fixed();
    let mut rt = CSharpRuntime::new().expect("dotnet required");
...
#[tokio::test]
#[ignore = "requires dotnet SDK in PATH"]
async fn test_csharp_fixed_mode_iterations_100() {
    let mut suite = SuiteIR::new("snap".to_string());
    suite.mode = BenchMode::Fixed;
