//! Go embedded code hover via gopls

use poly_bench_lsp_traits::{EmbeddedHoverContext, EmbeddedHoverProvider};
use poly_bench_syntax::Lang;
use tower_lsp::lsp_types::Hover;

/// Go embedded hover provider
pub(crate) struct GoEmbeddedHoverProvider;

impl EmbeddedHoverProvider for GoEmbeddedHoverProvider {
    fn lang(&self) -> Lang {
        Lang::Go
    }
            ...
            Ok(Some(mut hover)) => {
                if let Some(ref go_range) = hover.range {
                    if let Some(bench_start_offset) =
                        virtual_file.virtual_to_bench(go_range.start.line, go_range.start.character)
                    {
                        if let Some(bench_end_offset) =
                            virtual_file.virtual_to_bench(go_range.end.line, go_range.end.character)
                        {
...
pub(crate) static GO_EMBEDDED_HOVER_PROVIDER: GoEmbeddedHoverProvider = GoEmbeddedHoverProvider;
