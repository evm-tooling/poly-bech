//! Deterministic snapshot tests for Go runtime execution.
...
fn minimal_suite_fixed() -> (BenchmarkSpec, SuiteIR) {
    ...
    spec.implementations.insert(Lang::Go, "len(data)".to_string());
...
#[tokio::test]
#[ignore = "requires Go toolchain in PATH"]
async fn test_go_fixed_mode_execution() {
...
#[tokio::test]
#[ignore = "requires Go toolchain in PATH"]
async fn test_go_fixed_mode_iterations_100() {
    let mut suite = SuiteIR::new("snap".to_string());
    suite.mode = BenchMode::Fixed;
