//! Deterministic snapshot tests for TypeScript runtime execution.
...
use poly_bench_runtime_traits::{Measurement, Runtime};
use runtimes_ts::JsRuntime;
...
fn minimal_suite_fixed() -> (BenchmarkSpec, SuiteIR) {
    ...
    spec.implementations.insert(Lang::TypeScript, "data.length".to_string());
...
#[tokio::test]
#[ignore = "requires Node.js in PATH"]
async fn test_ts_fixed_mode_execution() {
    let (spec, suite) = minimal_suite_fixed();
    let mut rt = JsRuntime::new().expect("Node.js required");
...
#[tokio::test]
#[ignore = "requires Node.js in PATH"]
async fn test_ts_fixed_mode_iterations_100() {
    let mut suite = SuiteIR::new("snap".to_string());
    suite.mode = BenchMode::Fixed;
