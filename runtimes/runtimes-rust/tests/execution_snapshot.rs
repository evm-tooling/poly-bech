//! Deterministic snapshot tests for Rust runtime execution.
...
fn minimal_suite_fixed() -> (BenchmarkSpec, SuiteIR) {
    ...
    spec.implementations.insert(Lang::Rust, "data.len()".to_string());
...
#[tokio::test]
#[ignore = "requires Rust toolchain (cargo) in PATH"]
async fn test_rust_fixed_mode_execution() {
...
#[tokio::test]
#[ignore = "requires Rust toolchain (cargo) in PATH"]
async fn test_rust_fixed_mode_iterations_100() {
    let mut suite = SuiteIR::new("snap".to_string());
    suite.mode = BenchMode::Fixed;
