//! Deterministic snapshot tests for Python runtime execution.
...
use runtimes_python::PythonRuntime;
...
fn minimal_suite_fixed() -> (BenchmarkSpec, SuiteIR) {
    ...
    spec.implementations.insert(Lang::Python, "len(data)".to_string());
...
async fn test_python_fixed_mode_execution() {
    let (spec, suite) = minimal_suite_fixed();
    let mut rt = PythonRuntime::new().expect("Python required");
...
#[tokio::test]
#[ignore = "requires Python in PATH"]
async fn test_python_fixed_mode_iterations_100() {
    let mut suite = SuiteIR::new("snap".to_string());
    suite.mode = BenchMode::Fixed;
