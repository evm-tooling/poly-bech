//! Zig embedded code diagnostics via ZLS.
...
pub(crate) struct ZigEmbeddedDiagnosticProvider;

impl EmbeddedDiagnosticProvider for ZigEmbeddedDiagnosticProvider {
    fn check_blocks(
        &self,
        ...
        let client = match ctx.get_lsp_client(poly_bench_dsl::Lang::Zig, &module_root) {
            Some(c) => c,
            None => return diagnostics,
        };
        ...
        {
            tracing::warn!("[zig-diagnostics] failed to sync virtual file: {}", e);
            return diagnostics;
        }
        ...
        match client.request_diagnostics(virtual_file.uri()) {
            ...
            Err(e) => {
                tracing::debug!("[zig-diagnostics] request failed: {}", e);
            }
        }
...
pub(crate) static ZIG_EMBEDDED_DIAGNOSTIC_PROVIDER: ZigEmbeddedDiagnosticProvider =
    ZigEmbeddedDiagnosticProvider;

pub(crate) struct ZigEmbeddedDiagnosticSetup;

impl EmbeddedDiagnosticSetup for ZigEmbeddedDiagnosticSetup {
    fn lang(&self) -> Lang {
        Lang::Zig
    }

    fn prepare(&self, module_root: &str, ctx: &dyn EmbeddedDiagnosticContext) {
        ctx.ensure_ready(Lang::Zig, module_root);
    }
}

pub(crate) static ZIG_EMBEDDED_DIAGNOSTIC_SETUP: ZigEmbeddedDiagnosticSetup =
    ZigEmbeddedDiagnosticSetup;
