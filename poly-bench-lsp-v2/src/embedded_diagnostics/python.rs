...
pub fn check_python_blocks(virtual_file: &dyn VirtualFile) -> Vec<EmbeddedDiagnostic> {
...
fn run_pyright(file_path: &str, content: &str) -> Option<Vec<EmbeddedDiagnostic>> {
    ...
    for d in diagnostics {
        ...
        let length = if line == end_line {
            end_char.saturating_sub(character)
        } else {
            character.saturating_sub(character) // placeholder, use message len
        };
