//! Init runtime dirs test: verify poly-bench init correctly sets up .polybench/runtime-env/ for
//! each runtime. No build step; init-only. Run with: cargo test -p integration-tests
//! init_runtime_dirs
...
fn init_runtime_dirs() -> Result<()> {
    ...
    with_temp_project_init(languages, "pb-project", |project_path| {
        ...
        for lang in [Lang::Go, Lang::TypeScript, Lang::Rust, Lang::Python, Lang::Zig] {
            ...
            assert!(env_path.exists(), ".polybench/runtime-env/{} should exist", lang.as_str());
        }
        ...
        assert!(
            runtime_env(project_path, Lang::TypeScript).join("package.json").exists(),
            "ts/package.json should exist"
        );
        assert!(
            runtime_env(project_path, Lang::Rust).join("Cargo.toml").exists(),
            "rust/Cargo.toml should exist"
        );
        assert!(
            runtime_env(project_path, Lang::Python).join("requirements.txt").exists(),
            "python/requirements.txt should exist"
        );
        assert!(
            runtime_env(project_path, Lang::Zig).join("build.zig").exists(),
