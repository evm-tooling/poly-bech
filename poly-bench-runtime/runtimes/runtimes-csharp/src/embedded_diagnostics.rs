//! C# embedded code diagnostics via OmniSharp.
...
pub(crate) struct CSharpEmbeddedDiagnosticProvider;

impl EmbeddedDiagnosticProvider for CSharpEmbeddedDiagnosticProvider {
    fn check_blocks(
        &self,
        ...
        {
            tracing::warn!("[csharp-diagnostics] failed to sync virtual file: {}", e);
            return diagnostics;
        }
        ...
        match client.request_diagnostics(virtual_file.uri()) {
            ...
            Err(e) => {
                tracing::debug!("[csharp-diagnostics] request failed: {}", e);
            }
        }
...
pub(crate) static CSHARP_EMBEDDED_DIAGNOSTIC_PROVIDER: CSharpEmbeddedDiagnosticProvider =
    CSharpEmbeddedDiagnosticProvider;

pub(crate) struct CSharpEmbeddedDiagnosticSetup;

impl EmbeddedDiagnosticSetup for CSharpEmbeddedDiagnosticSetup {
    fn lang(&self) -> Lang {
        Lang::CSharp
    }

    fn prepare(&self, module_root: &str, ctx: &dyn EmbeddedDiagnosticContext) {
        ctx.ensure_ready(Lang::CSharp, module_root);
    }
}

pub(crate) static CSHARP_EMBEDDED_DIAGNOSTIC_SETUP: CSharpEmbeddedDiagnosticSetup =
    CSharpEmbeddedDiagnosticSetup;
