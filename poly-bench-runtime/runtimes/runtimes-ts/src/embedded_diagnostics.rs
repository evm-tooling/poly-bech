...
/// TypeScript embedded diagnostic provider
pub(crate) struct TsEmbeddedDiagnosticProvider;
...
pub(crate) static TS_EMBEDDED_DIAGNOSTIC_PROVIDER: TsEmbeddedDiagnosticProvider =
    TsEmbeddedDiagnosticProvider;

/// TypeScript embedded diagnostic setup - delegates to context
pub(crate) struct TsEmbeddedDiagnosticSetup;
...
pub(crate) static TS_EMBEDDED_DIAGNOSTIC_SETUP: TsEmbeddedDiagnosticSetup =
    TsEmbeddedDiagnosticSetup;

/// Check TypeScript blocks by running tsc directly
fn check_ts_blocks(virtual_file: &dyn VirtualFile) -> Vec<EmbeddedDiagnostic> {
