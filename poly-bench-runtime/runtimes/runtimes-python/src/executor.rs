...
fn generate_standalone_script(spec: &BenchmarkSpec, suite: &SuiteIR) -> Result<String> {
    ...
    if is_async {
        script.push_str("async def __polybench_bench():\n");
    } else {
    ...
    if is_async {
        script.push_str("async def __polybench_run():\n");
    } else {
    ...
    if spec.warmup_time_ms > 0 {
        ...
        if is_async {
            if use_sink {
                script.push_str("        __polybench_sink = await __polybench_bench()\n");
            } else {
                script.push_str("        await __polybench_bench()\n");
            }
        } else if use_sink {
    ...
    if is_async {
        ...
        script.push_str(&format!("    sample_cap = {}\n", sample_cap));
        script.push_str("    total_iterations = 0\n");
        script.push_str("    total_ns = 0.0\n");
        script.push_str("    samples = []\n");
        script.push_str("    successful_results = []\n");
        script.push_str("    error_samples = []\n");
        script.push_str("    successful_count = 0\n");
        script.push_str("    error_count = 0\n");
        if is_fixed_cap {
            script.push_str(&format!("    for _ in range(sample_cap):\n"));
        } else {
        ...
        script.push_str("            successful_count += 1\n");
        if use_sink {
            script.push_str("            if len(successful_results) < sample_cap:\n");
            script.push_str("                try:\n");
            script.push_str("                    r = json.dumps(__polybench_sink, default=str)\n");
            script.push_str("                    if r != 'null': successful_results.append(r)\n");
            script.push_str("                except: pass\n");
        }
        script.push_str("        except Exception as e:\n");
        script.push_str("            error_count += 1\n");
        script.push_str("            if len(error_samples) < sample_cap:\n");
        script.push_str("                error_samples.append(str(e)[:240])\n");
        script.push_str("        elapsed = time.perf_counter_ns() - start\n");
        script.push_str("        total_ns += elapsed\n");
        script.push_str("        total_iterations += 1\n");
        script.push_str("        if len(samples) < sample_cap:\n");
        script.push_str("            samples.append(elapsed)\n");
        script.push_str("        elif sample_cap > 0:\n");
        script.push_str("            r = random.randint(0, total_iterations)\n");
        script.push_str("            if r < sample_cap: samples[r] = elapsed\n");
    ...
    if is_async {
        script.push_str("        , \"successfulCount\": successful_count\n");
        script.push_str("        , \"errorCount\": error_count\n");
        script.push_str("        , \"successfulResults\": successful_results\n");
        script.push_str("        , \"errorSamples\": error_samples\n");
    }
    script.push_str("    }\n");
    script.push_str("    print(json.dumps(result))\n\n");
    if is_async {
        script.push_str("asyncio.run(__polybench_run())\n");
    } else {
