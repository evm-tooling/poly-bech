...
This workspace contains language-specific runtime crates for poly-bench. Each runtime implements the plugin interface defined in `poly-bench-traits`.

## Structure

```
poly-bench-runtime/runtimes/
...
1. **Runtime** – Implements `poly_bench_traits::Runtime`:
   - `compile_check`, `run_benchmark`, `initialize`, `shutdown`
   - `generate_check_source` for validation

2. **RuntimeFactory** – Implements `poly_bench_traits::RuntimeFactory`:
   - `create(config) -> Box<dyn Runtime>`
   - Registered in `poly-bench-runtime` registry

3. **ErrorMapper** – Implements `poly_bench_traits::ErrorMapper`:
...
- **poly-bench-traits** – `Runtime`, `RuntimeFactory`, `ErrorMapper`, `LangDisplayInfo`, `Measurement`, `RuntimeConfig`
