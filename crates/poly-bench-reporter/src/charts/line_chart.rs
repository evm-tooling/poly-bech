...
use super::{
    ...
    calculate_geo_mean, count_wins,
    DEFAULT_WIDTH, DEFAULT_MARGIN_TOP, DEFAULT_MARGIN_BOTTOM,
    DEFAULT_MARGIN_LEFT, DEFAULT_MARGIN_RIGHT,
    GO_COLOR, TS_COLOR, TEXT_COLOR, TEXT_MUTED, TEXT_SECONDARY, BORDER_COLOR,
};
...
pub fn generate(
    ...
    let stats_box_height = if directive.show_stats || directive.show_distribution || directive.show_geo_mean { 80 } else { 0 };
    ...
    // Draw Go line
    draw_series(&mut svg, &go_points, &bench_names, chart_width, chart_height, max_value, GO_COLOR, directive.show_stats, time_unit, precision);
    
    // Draw TS line
    draw_series(&mut svg, &ts_points, &bench_names, chart_width, chart_height, max_value, TS_COLOR, directive.show_stats, time_unit, precision);
