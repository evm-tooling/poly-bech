...
pub fn generate(
    ...
    let benchmarks_with_both: Vec<_> = filtered.iter()
        .filter(|b| {
            let has_go = b.measurements.get(&Lang::Go).map(|m| m.total_nanos > 0).unwrap_or(false);
            let has_ts = b.measurements.get(&Lang::TypeScript).map(|m| m.total_nanos > 0).unwrap_or(false);
            has_go || has_ts
        })
        .collect();
    ...
    for (i, bench) in benchmarks_with_both.iter().enumerate() {
        ...
        // Get Go and TS times
        let go_time = bench.measurements.get(&Lang::Go)
            .map(|m| m.total_nanos as f64)
            .unwrap_or(0.0);
        let ts_time = bench.measurements.get(&Lang::TypeScript)
            .map(|m| m.total_nanos as f64)
            .unwrap_or(0.0);
        ...
        if !compact {
            let winner_color = if go_faster { GO_COLOR } else if ts_faster { TS_COLOR } else { TEXT_MUTED };
            let winner_label = if go_faster { "Go" } else if ts_faster { "TS" } else { "=" };
            svg.push_str(&format!(
    ...
    if (directive.show_stats || directive.show_distribution || directive.show_geo_mean) && !compact {
        ...
        if directive.show_geo_mean {
            let geo_label = if geo_mean >= 1.0 {
                format!("Go is {:.2}x faster on average (geometric mean)  |  Total benchmarks: {}", geo_mean, filtered.len())
            } else {
